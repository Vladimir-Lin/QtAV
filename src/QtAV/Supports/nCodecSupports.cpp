...
N::CodecSupports:: CodecSupports (QWidget * parent,Plan * p)
                 : TreeWidget    (          parent,       p)
{
  WidgetClass   ;
  Configure ( ) ;
...
N::CodecSupports::~CodecSupports (void)
{
}

void N::CodecSupports::Configure(void)
{
  NewTreeWidgetItem            ( head                             ) ;
  setWindowTitle               ( tr("Audio/Video codec supports") ) ;
  ...
  head -> setText              ( 1,tr("Detail name")              ) ;
  head -> setText              ( 2,tr("Name"       )              ) ;
  head -> setText              ( 3,tr("Channel"    )              ) ;
  head -> setText              ( 4,tr("Type"       )              ) ;
  setAllAlignments             ( head,Qt::AlignCenter             ) ;
  setFont                      ( head,Fonts::ListView             ) ;
  setHeaderItem                ( head                             ) ;
  plan -> setFont              ( this                             ) ;
  plan -> setFont              ( header() , Fonts::ListView       ) ;
  Detect                       (                                  ) ;
}

QString N::CodecSupports::TypeName(int codectype)
{
  switch ( codectype )           {
...
void N::CodecSupports::AddItem(QString name,int ID)
{
  AVCodec * codec = NULL                             ;
  ////////////////////////////////////////////////////
  ...
  if (codec!=NULL)                                   {
    ...
    it -> setText       ( 0,name                  )  ;
    it -> setText       ( 1,codec->long_name      )  ;
    it -> setText       ( 2,codec->name           )  ;
    it -> setText       ( 3,TypeName(codec->type) )  ;
    it -> setText       ( 4,tr("Decoder")         )  ;
    it -> setForeground ( 4,QColor(0,0,255)       )  ;
    addTopLevelItem     ( it                      )  ;
  }                                                  ;
  ////////////////////////////////////////////////////
  ...
  if (codec!=NULL)                                   {
    ...
    it->setText         ( 2,codec->name           )  ;
    it->setText         ( 3,TypeName(codec->type) )  ;
    it->setText         ( 4,tr("Encoder")         )  ;
    it->setForeground   ( 4,QColor(255,0,0)       )  ;
    addTopLevelItem     ( it                      )  ;
  }                                                  ;
}

void N::CodecSupports::Detect(void)
{
  clear ( ) ;
